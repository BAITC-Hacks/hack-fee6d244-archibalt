...
interface Task {
  ...
  questions: Question[];                   // батч-режим: ≥3 от AI сразу; пошаговый (mode=dynamic): по одному, минимум 3, предел 10 (more — 12)
  ai_mode: "openai" | "mock";
...
Альтернатива батч-режиму (все вопросы сразу — остаётся как есть, фронт выбирает любой). AI задаёт по одному вопросу и выбирает следующий по смыслу черновика и уже данных ответов: сначала суть проекта, процесс, пользователи и ожидаемая польза. Останавливается, когда сведений достаточно для обсуждения подходящего результата, но **не раньше 3 вопросов**. **10 вопросов — предохранитель**, а не обязательная длина диалога. Частичный ответ можно уточнить, если модель видит конкретный пробел в `missing_fields`; не больше 2 вопросов на тему. Ответ без сведений (`rating.IsNoise`, например приветствие или «не знаю») не заполняет поле карточки. Явный пропуск (`answer: ""`) уважает выбор пользователя: тему повторно не спрашиваем, даже если сведения отсутствуют. По явному запросу «спросить ещё» диалог продолжается по оставшимся темам — **не больше 12 вопросов всего**, с ранним завершением, если уточнять больше нечего.

1. `POST /api/tasks` `{ draft_text, industry, mode: "dynamic" }` (или `?mode=dynamic`) → `Task`, в `questions` — один вопрос (id 1).
2. `POST /api/tasks/{id}/next-question` `{ answer: "..." }` — ответ записывается в последний вопрос, если он без ответа (`answer: ""` = «пропустить», сохраняется как ""; без поля `answer` ничего не записывается). Запрос **без `answer`** (или с `more: true`) — явное «спросить ещё»: AI проверяет оставшиеся темы и предлагает следующий вопрос, если есть что уточнить; потолок 12. Затем AI выбирает следующий:

```ts
interface NextQuestion {
  done: boolean;
  question?: Question;          // при done=false: новый вопрос уже добавлен в task.questions (id = max+1)
  reason?: string;              // при done=true: причина завершения или следующий шаг; лимит не означает полноту сведений
  asked: number;                // сколько вопросов задано (длина task.questions)
  missing_fields: FieldKey[];   // смысловые пробелы по оценке модели; могут оставаться после частичного ответа или пропуска
...
3. При `done: true` вопросы не добавляются. Вызов без `answer` / с `more: true` («Спросить ещё») может дать ещё вопрос по оставшимся темам; если уточнять нечего, снова вернёт `done`. На пределе 10 вопросов (12 по `more`) сервер возвращает `done` с предложением собрать промежуточную карточку и дописать неясные детали в редакторе, не объявляя сведения полными. Карточку собирает `POST /api/tasks/{id}/answers` с телом `{}` — из сохранённых ответов, через AI + Guard, как в батч-режиме.

Права: только заявитель (без токена 401, чужой — 403, у задачи без заявителя — 403); исключение для анонимных черновиков описано в разделе «Разговор без входа». Ошибки: неверный JSON → 400, нет задачи → 404. AI: строгая JSON-схема `{done, reason, question: {text, field_key, input_type, suggestions} | null, missing_fields}`, промпт — `GET /api/ai` → `prompt_next_question`; минимум 3, предел 10 (по `more` — 12), уважение пропуска и не больше 2 вопросов на тему применяются поверх ответа модели; при ошибке — повтор и детерминированный mock. Замер живого прогона (gpt-6-sol): ~6–9 с на шаг.
...
## Визуальный концепт первого результата (необязательно)

Картинка «как может выглядеть результат» для обсуждения, не обещание объёма. Фронт запускает генерацию сам после `apply-result`; на обязательный путь (диалог → карточка → публикация) не влияет: ошибка/нет ключа не блокирует «Собрать карточку».

- `POST /api/tasks/{id}/visual` — только заявитель (как `apply-result`: 401/403). Синхронно генерирует (OpenAI Images `gpt-image-1`, 1024×1024, `quality=low`, таймаут 60 с) и сохраняет в `task_visuals` (одна на задачу, повтор перезаписывает). Поля задачи не меняются. Ответ `200 { "url": "/api/tasks/7/visual.png?ts=…", "ai_mode": "openai"|"mock", "model": "gpt-image-1", "prompt": "…", "latency_ms": 10913 }`. Без `OPENAI_API_KEY` — SVG-заглушка «Пример концепта (mock)», `ai_mode: "mock"`. Сбой/квота/таймаут → `502 { "error": "не получилось нарисовать концепт: …" }`.
- `GET /api/tasks/{id}/visual.png` — картинка (`image/png`, у mock `image/svg+xml`), `Cache-Control: no-store`, заголовок `X-AI-Mode`. Заявителю (Bearer) — всегда; остальным — только у опубликованной задачи; иначе и при отсутствии картинки — 404.
- `GET /api/tasks/{id}` отдаёт `has_visual: bool` (в каталоге и прочих ответах всегда `false`).
- Промпт строится из полей: название, потребность, ожидаемый результат, пользователи; тип картинки по содержанию (ключевой экран / схема процесса / пример отчёта), стиль «чистый продуктовый концепт, без мелкого текста и логотипов».
