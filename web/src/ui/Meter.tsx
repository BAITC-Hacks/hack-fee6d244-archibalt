...
/** Плавно доводит число до нового значения; без анимации при prefers-reduced-motion. */
export function useAnimatedNumber(target: number, duration = 700) {
  const [shown, setShown] = useState(target)
  const from = useRef(target)
  useEffect(() => {
    const start = from.current
    if (start === target || window.matchMedia?.('(prefers-reduced-motion: reduce)').matches) { from.current = target; setShown(target); return }
...
export function Meter({ value, label = 'Готовность задачи', showValue = true }: { value: number; label?: string; showValue?: boolean }) {
  ...
  return <div className="ui-meter">
    {showValue && <div className="ui-meter-head"><span className="ui-meter-value">{shown}</span><span className="ui-meter-of">из 100</span></div>}
