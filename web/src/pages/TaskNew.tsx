...
export const DRAFT_PLACEHOLDER = 'Например: языковой центр вырос из Excel, заявки теряются в WhatsApp, руководству не видна картина.'
...
export function TaskNew() {
  ...
  async function create(token: string, contact: string) {
    setBusy(true); setError('')
    try {
      const task = await api<Task>('/tasks', withToken(token, json('POST', { draft_text: draft, industry, owner_contact: contact })))
  ...
  function submit(event: FormEvent) {
    ...
    void create(business.token, business.contact)
  }
  return <div className="container flow-page">
    ...
    <div className="flow-layout">
      <form className="paper-form" onSubmit={submit} noValidate>
        ...
        <Field label="Краткое описание" required error={draftError || undefined} hint={savedAt ? 'Черновик сохранён на этом устройстве.' : 'Сведения, которых нет, AI не придумает: их можно добавить позже.'}>
