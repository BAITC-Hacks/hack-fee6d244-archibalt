...
import { Button, Chips, Field, Input, Textarea } from '../ui'
...
export function TaskNew() {
  const location = useLocation(); const { openAgentChat } = useAgentChat()
  const { business, requestLogin } = useSession()
  ...
  const [draftError, setDraftError] = useState(''); const [industryError, setIndustryError] = useState(''); const [savedAt, setSavedAt] = useState(false)
  ...
  /** Задачу создаёт диалог с агентом: там же вопросы по одному и живая карточка. */
  const start = () => openAgentChat({ draftText: draft.trim(), industry: industry.trim() })
  function submit(event: FormEvent) {
    ...
    if (!ok) return
    if (!business) { requestLogin('business', start); return }
    start()
  }
    ...
    <div className="flow-layout">
      <form className="paper-form" onSubmit={submit} noValidate>
        <Field label="Краткое описание" required error={draftError || undefined} hint={<DraftHint saved={savedAt} />}>
          <Textarea minRows={5} value={draft} onChange={event => { setDraft(event.target.value); setDraftError('') }} placeholder={DRAFT_PLACEHOLDER} />
        </Field>
        <Field label="Тема" required error={industryError || undefined}>
          <Input value={industry} onChange={event => { setIndustry(event.target.value); setIndustryError('') }} placeholder="Например, образование" />
        </Field>
        <Chips label="Темы из каталога" options={data.industries} value={industry} onPick={value => { setIndustry(value); setIndustryError('') }} />
        <Button type="submit" variant="primary" size="lg" className="form-submit">Уточнить с агентом →</Button>
